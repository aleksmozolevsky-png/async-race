  ...
  return (
    <div style={{ padding: '20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ position: 'relative', width: '100%' }}>
        ...
        {winnerName && (
          <div style={{
            ...
            <div style={{ margin: '10px 0', fontSize: '28px', textTransform: 'uppercase' }}>{winnerName}</div>
      ...
      <div style={{ marginTop: '20px', display: 'flex', gap: '10px' }}>
        <button type="button" disabled={currentPage === 1} onClick={() => handlePageChange(currentPage - 1)}>Prev</button>
        <button type="button" disabled={currentPage * 7 >= totalCount} onClick={() => handlePageChange(currentPage + 1)}>Next</button>
      </div>
    </div>
  );
};